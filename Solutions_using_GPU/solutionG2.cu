...
// Parallelized find function, threads_per_block = 512
...
__global__ void find(int i, int *options, int options_size, int k, int *v, int *v_size, int *cnt)
{
    ...
    i++;
    if(k - i + 1 > intersec_size) return;
    if(i == k)
    {
        atomicAdd(cnt, intersec_size);
        return;
    }

    // Launching kernel inside kernel
    int blocks = intersec_size / threads_per_block + 1;
    find<<<blocks,threads_per_block>>>(i, intersec, intersec_size, k, v, v_size, cnt);
...
__global__ void solve(int *i, int *options, int *options_size, int *k, int *v, int *v_size, int *cnt)
{
    (*i) = 1;
    int threads_per_block = 512;
    int blocks = (*options_size) / threads_per_block + 1;
    find<<<blocks,threads_per_block>>>((*i), options, (*options_size), (*k), v, v_size, cnt);
}
