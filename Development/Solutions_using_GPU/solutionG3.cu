...
// Find made iterative, one block per subtree
...
__global__ void find_iterative(int *k, bool *G_linear, int *imp, int *imp_size, int *cnt)
{
    __shared__ int rootIdx;
    __shared__ int root;
    __shared__ int lvl;
    __shared__ bool lvl_vertices[(*k) + 1][(*imp_size)];
    __shared__ int cur_vertex_id[(*k) + 1];
    if(threadIdx.x == 0)
    {
        rootIdx = blockIdx.x;
        root = imp[rootIdx];
    ...
    __shared__ bool adj_vertex[(*imp_size)];
    __shared__ int num_lvl_vertices[(*k) + 1];
    __syncthreads();
    ...
    while(cur_vertex_id[lvl] < (*imp_size))
    {
        ...
        // lvl_vertices[l + 1] = intersec(lvl_vertices[lvl], adj_vertex));
        int idx = threadIdx.x;
        while(idx < (*imp_size))
        {
            ...
            {
                atomicAdd(&(num_lvl_vertices[lvl + 1]), 1);
            }
            idx += blockDim.x;
        }
        __syncthreads();
...
//------------------------ ALGORITHM Starts ----------------------------> 
    ...
    */
    ...
    int block_size = 1024;
    find_iterative<<<imp_size, block_size>>>(d_k, d_G_linear, d_imp, d_imp_size, d_cnt);
